        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 'auto' }}>
          ...
          {!currentUser ? (
            ...
            <div style={{ position: 'relative' }}>
              ...
              {userMenuOpen && (
                <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 8px)', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, minWidth: 180, boxShadow: 'var(--shadow-lg)', overflow: 'hidden', zIndex: 200 }}>
                  ...
                  <button onClick={() => { logout(); setUserMenuOpen(false) }} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '12px 16px', color: 'var(--red)', background: 'none', border: 'none', borderTop: '1px solid var(--border)', fontSize: '0.875rem', cursor: 'pointer', fontFamily: 'inherit' }}>🚪 Déconnexion</button>
                </div>
              )}
            </div>
          )}
        ...
        <div style={{ background: '#0f172a', borderTop: '1px solid rgba(255,255,255,0.08)', padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          {!currentUser ? (
            ...
            <>
              ...
              <button onClick={() => { logout(); setMenuOpen(false) }} style={{ background: 'none', border: 'none', textAlign: 'left', color: 'var(--red)', padding: '12px 8px', fontSize: '0.95rem', cursor: 'pointer', fontFamily: 'inherit' }}>🚪 Déconnexion</button>
            </>
          )}
        </div>
      )}
