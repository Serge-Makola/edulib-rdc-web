...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (fbUser) => {
      ...
      if (fbUser) {
        ...
        if (snap.exists()) setCurrentUser({ uid: fbUser.uid, ...snap.data() } as User)
      } else {
        setCurrentUser(null)
      }
      setIsLoading(false)
  ...
  async function register(name: string, email: string, pass: string, role: string, filiere: string) {
    ...
    await setDoc(doc(db, 'users', cred.user.uid), { name, email, role, filiere, boughtIds: [], createdAt: Date.now(), uid: cred.user.uid })
  ...
  return (
    <AuthContext.Provider value={{ currentUser, firebaseUser, isAdmin, isLoading, login, register, logout, resetPassword }}>
      {children}
